...
**Требования:** Ubuntu 22.04/24.04, Python 3.12+.

### Установка

```bash
# Клонировать репозиторий
git clone ssh://git@ssh.github.com:443/<логин>/<репо>.git
cd <репо>
...
```bash
mkdir -p carbot/config
nano carbot/config/.env.server
```
...
[Service]
User=root
WorkingDirectory=/root/<репо>
ExecStart=/root/<репо>/venv/bin/python main.py
...
sudo systemctl enable carwatch
sudo systemctl start carwatch
```
...
```bash
# Статус
sudo systemctl status carwatch

# Логи в реальном времени
sudo journalctl -u carwatch -f

# Перезапуск
sudo systemctl restart carwatch

# Остановка
sudo systemctl stop carwatch
```
